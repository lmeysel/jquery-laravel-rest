...
var rest = function ($) {
	...
	function e(sender) {
		var cont = $('<div>').appendTo($('body')).css({ 'width': '100%', 'height': '100%', 'position': 'fixed', 'top': 0, 'left': 0, 'z-index': 99999, 'background-color': '#ffe6e6' });
		var text = $('<a>').appendTo(cont).css({ 'color': 'red', 'font-size': '2em', 'cursor': 'pointer', position: 'absolute', 'right': '10px', 'top': '10px' }).html('<i class="fa fa-times"></i>&nbsp;<strong>Close</strong>').click(function () {
			cont.remove();
		});
		var iframe = $('<iframe>').appendTo(cont).css({
			'position': 'absolute',
   ...
   */
	...
	return function () {
		...
		if (a.csrf_token) arguments[1] = $.extend({ '_token': a.csrf_token }, arguments[1]);
